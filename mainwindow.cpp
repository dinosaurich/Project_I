...
void Fill_Field(QPainter* p, QPixmap* map,QImage* image, int cellPosX, int cellPosY, int onePixDistance, int TxPower, int antGain, float freq)
{
    for(int i = 0; i < maxX; i++)
    {
        for(int j = 0; j < maxY; j++)
        {
            ...
            {
                //затухание
                sigPower=sigPower-NumObs[0]*WallLoss(0, freq)-NumObs[1]*WallLoss(1, freq)-NumObs[2]*WallLoss(2, freq)-NumObs[3]*WallLoss(3, freq);
            }
    ...
    : QMainWindow(parent)
{
    ...
    Wall Fence; //забор
    Fence.wood = true;
    Fence.wall(&p, 100, 200, 100, 50);

    Wall Fence1; //забор
    Fence.wood = true;
    Fence.wall(&p, 200, 300, 50, 150);
